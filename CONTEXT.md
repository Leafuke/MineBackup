...
- **日志记录（Log Record）**：对一次可观察事件的不可变描述，由级别、类别、稳定事件 ID、消息、会话、来源和上下文共同表达。_避免称为_：控制台行、日志文本。
- **稳定事件 ID（Stable Event ID）**：跨语言和呈现方式保持不变、用于识别事件语义的机器可读名称；本地化消息不是事件身份。
- **日志上下文（Log Context）**：在一次操作范围内自动附加到所有日志记录的关联身份，例如 operation_id、config_id、world、task 和 request_id。_避免称为_：消息前缀。
- **会话日志（Session Log）**：当前 MineBackup 进程会话中可供界面和诊断使用的结构化日志记录集合；其有界保留不代表删除本地文件日志。
- **本地轮转日志（Local Rotating Log）**：保留真实本机诊断信息、仅用于本机排障的持久日志；它不是默认可分享内容。
- **诊断日志（Diagnostic Log）**：经用户确认导出、只包含运行元数据和仍保留会话日志，并对已知路径、远端位置及 URL 敏感部分脱敏的可分享副本。
