...
string(JSON config_id GET "${manifest_json}" configs 0 configId)
string(JSON job_id GET "${manifest_json}" jobs 0 jobId)
run_success(config_show_json --data-dir "${PROFILE}" config show --config "${config_id}")
string(JSON shown_config_id GET "${config_show_json}" data config configId)
if(NOT shown_config_id STREQUAL config_id)
    message(FATAL_ERROR "config show did not return the requested stable identity: ${config_show_json}")
endif()
run_success(job_list_json --data-dir "${PROFILE}" job list)
string(JSON listed_jobs LENGTH "${job_list_json}" data jobs)
if(NOT listed_jobs EQUAL 1)
    message(FATAL_ERROR "job list did not return the applied Job: ${job_list_json}")
endif()
run_success(job_show_json --data-dir "${PROFILE}" job show --job "${job_id}")
string(JSON shown_job_id GET "${job_show_json}" data job jobId)
if(NOT shown_job_id STREQUAL job_id)
    message(FATAL_ERROR "job show did not return the requested stable identity: ${job_show_json}")
endif()
