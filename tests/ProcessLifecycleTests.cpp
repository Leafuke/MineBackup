...
int Scenario(const string& name, const filesystem::path& executable, const filesystem::path& root) {
    ...
    filesystem::create_directories(root);
    if (name == "failure") {
        const auto result = ProcessRunner::Run(Spec(executable, root / "a", L"fail"));
        test.Expect(result.status == ProcessStatus::ExitedWithError && result.exitCode == 7,
            "normal cleanup must preserve nonzero child exit codes");
    }
    else if (name == "restore-signal") {
        struct sigaction before{}, custom{}, after{};
        custom.sa_handler = +[](int) {};
        sigemptyset(&custom.sa_mask);
        sigaddset(&custom.sa_mask, SIGUSR1);
        test.Expect(sigaction(SIGTERM, &custom, &before) == 0, "install fixture disposition");
        {
            CliSignalHandler signals;
            raise(SIGINT);
            test.Expect(Until([&] { return signals.Token().stop_requested(); }),
                "first signal must request cooperative cancellation");
        }
        test.Expect(sigaction(SIGTERM, nullptr, &after) == 0
            && after.sa_handler == custom.sa_handler && sigismember(&after.sa_mask, SIGUSR1) == 1,
            "signal handler lifetime must restore previous disposition and mask");
        sigaction(SIGTERM, &before, nullptr);
    }
    else if (name == "both" || name == "first-exits") {
...
#ifndef _WIN32
    vector<wstring> cases{L"both", L"first-exits", L"normal", L"cancel", L"timeout", L"failure", L"restore-signal"};
#ifdef MINEBACKUP_PROCESS_TEST_HOOKS
