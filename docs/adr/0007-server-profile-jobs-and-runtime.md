...
5. Profile Runtime 是备份、校验、还原、Job 和 KnotLink 的唯一协调边界。一次性 CLI 可临时持有；本地 `serve` 代理可长期持有，并通过同用户双向 IPC 接受原命令的透明转发。
...
`serve` 不成为第二套业务实现：直接 CLI、转发命令、GUI 和 KnotLink 入口复用同一备份/还原服务。热还原只在写世界前增加模组握手、save-and-exit、锁释放和 rejoin 协调；本地归档链在请求退出世界前完成校验。还原已经提交后发生的 rejoin 失败、超时或取消属于告警，不能反向改写为“文件还原失败”。
